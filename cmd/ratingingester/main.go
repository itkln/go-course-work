...
func main() {
	...
	ratingEvents, err := readRatingEvent(fileName)
	if err != nil {
		panic(err)
	}
...
func readRatingEvent(fileName string) ([]model.RatingEvent, error) {
...
func produceRatingEvents(topic string, producer *kafka.Producer, events []model.RatingEvent) error {
	for _, ratingEvent := range events {
		...
		if err := producer.Produce(&kafka.Message{TopicPartition: kafka.TopicPartition{Topic: &topic, Partition: kafka.PartitionAny}, Value: encodedEvent}, nil); err != nil {
			return err
		}
	}
	return nil
